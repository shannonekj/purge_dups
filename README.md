...
bin/pbcstat *.paf.gz 
bin/calcuts PB.stat > cutoffs 2>calcuts.log
```
...
**Outputs**
- .paf.gz == gzipped Pairwise mApping Format file (produced from minimap2)
- PB.base.cov == contig coverage in 150 bp sliding windows (produced from pbcstat)
- PB.stat == read depths (produced from pbcstat)
- cutoffs == autogenerated file with 6 estimated cutoff values to be used by purge_dups (produced from calcuts)
	* Value 1: estimated lower bound for **low** coverage -- contigs with an average coverge less than this value are likely junk contigs
	* Value 2: 
	* Value 3: 
	* Value 4: estimated **middle** coverage -- trasition between haploid and diploid
	* Value 5: 
	* Value 6: upper bound for **high** coverage -- contigs with an average coverage over this value are usually repeats
- calcuts.log ==
